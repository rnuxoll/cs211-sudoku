...
// starting from position set template
#ifndef GAME_CANDIDATE_SET_HXX
#define GAME_CANDIDATE_SET_HXX
...
private:
    // Maximum number of possible squares
    // TODO should this be (max-1) * (max-1)
    static constexpr std::size_t index_limit = max * max;
...
public:
    ...
    Candidate_set() noexcept = default;
    ...
    ///
    /// ```
    /// return Candidate_set{p1, p2};
    /// ```
    Candidate_set(std::initializer_list<value_type> elems);
    ...
    /// ...
    ///
    /// Candidate_set pos_set(v.begin(), v.end());
    /// ```
    template <class FWD_ITER>
    explicit Candidate_set(FWD_ITER begin, FWD_ITER end);
    ...
    /// set1 &= set2;
    /// ```
    Candidate_set& operator&=(Candidate_set that);
    ...
    Candidate_set operator&(Candidate_set) const;
    ...
    ///
    /// ```
    /// CHECK( ~Candidate_set() == Candidate_set::universe() );
    /// ```
    Candidate_set operator~() const;
    ...
    friend bool operator==(Candidate_set, Candidate_set);
...
// Iterator class, declared above inside Candidate_set and defined right here:
class Candidate_set::iterator
        : public std::iterator<
                std::input_iterator_tag,
                Candidate_set::value_type const>
{
    bits_t const *bits_;
    Candidate_set::value_type current_;
...
/// `Candidate_set`s).
bool
operator==(Candidate_set::iterator, Candidate_set::iterator);

/// Two iterators are unequal when they aren't equal.
bool
operator!=(Candidate_set::iterator, Candidate_set::iterator);

/// Prints a `Candidate_set`; suitable for debugging.
std::ostream&
operator<<(std::ostream&, Candidate_set);
...
template <class FWD_ITER>
Candidate_set::Candidate_set(FWD_ITER begin, FWD_ITER end)
{
    for (; begin != end; ++begin) {
        (*this)[*begin] = true;
    }
